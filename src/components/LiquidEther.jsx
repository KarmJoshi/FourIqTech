...
class Simulation {
  ...
  createShaderPass() {
    this.advection = new Advection({ cellScale: this.cellScale, fboSize: this.fboSize, dt: this.options.dt, src: this.fbos.vel_0, dst: this.fbos.vel_1 }, this.common);
    this.externalForce = new ExternalForce({ cellScale: this.cellScale, cursor_size: this.options.cursor_size, dst: this.fbos.vel_1 }, this.common, this.mouse);
    this.viscousPass = new Viscous({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, viscous: this.options.viscous, src: this.fbos.vel_1, dst: this.fbos.vel_viscous1, dst_: this.fbos.vel_viscous0, dt: this.options.dt }, this.common);
    this.divergencePass = new Divergence({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.vel_viscous0, dst: this.fbos.div, dt: this.options.dt }, this.common);
    this.poissonPass = new Poisson({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: this.fbos.div, dst: this.fbos.pressure_1, dst_: this.fbos.pressure_0 }, this.common);
    this.pressurePass = new PressurePass({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src_p: this.fbos.pressure_0, src_v: this.fbos.vel_viscous0, dst: this.fbos.vel_0, dt: this.options.dt }, this.common);
  }
  calcSize() {
  ...
  update() {
    ...
    if (this.options.isViscous) {
      vel = this.viscousPass.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt });
    }
    this.divergencePass.update({ vel });
    const pressure = this.poissonPass.update({ iterations: this.options.iterations_poisson });
...
class WebGLManager {
  constructor(props, common, mouse, paletteTex, bgVec4, rafRef, isVisibleRef) {
    this.props = props;
    this.common = common;
    this.mouse = mouse;
    this.rafRef = rafRef;
  ...
  pause() {
    this.running = false;
    if (this.rafRef.current) { cancelAnimationFrame(this.rafRef.current); this.rafRef.current = null; }
  }
  dispose() {
    try {
...
export default function LiquidEther({
  mouseForce = 20, cursorSize = 100, isViscous = false, viscous = 30, iterationsViscous = 8,
  iterationsPoisson = 8, dt = 0.014, BFECC = true, resolution = 0.35, isBounce = false,
  colors = ['#efbf04', '#efbf04'], style = {}, className = '', autoDemo = true,
  autoSpeed = 0.5, autoIntensity = 2.2, takeoverDuration = 0.25, autoResumeDelay = 1000,
  ...
  useEffect(() => {
    ...
    }, Common, Mouse, paletteTex, bgVec4, rafRef, isVisibleRef);
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      Object.assign(webgl.autoDriver, { enabled: autoDemo, speed: autoSpeed, resumeDelay: autoResumeDelay, rampDurationMs: autoRampDuration * 1000 });
